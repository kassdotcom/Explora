...
function input(sourceTxId, value, address = undefined) {
  return new Input(sourceTxId, 0, new Output(value, new ScriptPubKey("", address, "witness_v0_keyhash")));
}
...
test("an address funding two inputs of the same transaction puts both sources first, smaller or not", () => {
  const ranked = rankSources([input("aaa", 0.5, "bc1reused"), input("bbb", 2, "bc1other"), input("ccc", 1, "bc1reused")], [], []);
  assert.deepEqual(ranked, [["ccc", 1], ["aaa", 0.5], ["bbb", 2]]);
});

test("an address already on the map puts its source first", () => {
  const ranked = rankSources([input("aaa", 0.5, "bc1seen"), input("bbb", 2, "bc1new")], [], ["bc1seen"]);
  assert.deepEqual(ranked, [["aaa", 0.5], ["bbb", 2]]);
});

test("inputs without an address never count as reused", () => {
  const ranked = rankSources([input("aaa", 0.5), input("bbb", 2), input("ccc", 1)], [], []);
  assert.deepEqual(ranked, [["bbb", 2], ["ccc", 1], ["aaa", 0.5]]);
});
